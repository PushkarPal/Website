    ...
    test(`${viewport.name} preserves responsive geometry`, async ({ page }) => {
      ...
      const geometry = await page.evaluate((requestedSelectors) => {
        ...
        return {
          ...
          elements,
          ratios: {
            root: elements['#root'].width / viewportWidth,
            landing: elements['.landing-page'].width / viewportWidth,
            header: elements['.landing-header'].width / viewportWidth,
            nav: elements['.landing-nav'].width / viewportWidth,
            poster: posterRect.width / viewportWidth,
            shell: shellRect.width / viewportWidth,
          },
          carousel: {
            posterLeftInset: posterRect.left,
            posterRightInset: viewportWidth - posterRect.right,
            shellLeftInset: shellRect.left,
            shellRightInset: viewportWidth - shellRect.right,
            leftArrowGap: posterRect.left - leftArrowRect.right,
            rightArrowGap: rightArrowRect.left - posterRect.right,
            ...
            trackWidth: elements['.poster-track'].width,
            viewportWidth: posterRect.width,
          },
          category: {
      ...
      // Global document geometry must be viewport-bound.
      ...
      // Header/logo geometry remains bounded and centered.
      ...
      // Category structure is invariant across widths.
      ...
      // The carousel is a bounded composition. Its shell must never become the
      // full-width source of the arrow spacing problem, and every child stays in it.
      ...
      if (isMobile) {
        ...
        // continuously instead of depending on a fixed phone width.
